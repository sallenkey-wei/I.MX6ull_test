...
void clk_init(){
	...
	/* 2.设置PLL2各个PFD 以下操作没有使能各个PFD，可能是bootrom已经使能了吧*/
	...
	/* 设子uart时钟源频率为80MHz */
	CCM->CSCDR1 &= ~(1 << 6); /* 设置UART clock 使用pll3_80m */
	CCM->CSCDR1 &= ~(0x3f << 0); /* 设置UART clock 1分频 也就是频率为80MHz */

    /* 设置ECSPI_CLK_ROOT = 60MHz */
    CCM->CCSR &= ~(1 << 0); /* pll3_sw_clk 使用pll3 = 480MHz */
    CCM->CSCDR2 &= ~((1 << 18) | (0x3f << 19)); /* derive clock from pll3_60m , 并且1分频*/
}
